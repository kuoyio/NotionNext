...
function mapProperties(properties) {
  const typeMap = {
    ...
    [BLOG.NOTION_PROPERTY_NAME.type_link]: 'Link',
    [BLOG.NOTION_PROPERTY_NAME.type_movie]: 'Movie'
  }
...
export function adjustPageProperties(properties, NOTION_CONFIG) {
  ...
  if (properties.type === 'Post') {
    ...
    // 友链使用 slug 保存目标网址，不参与文章路径规则生成。
    properties.href = properties.slug ?? properties.id
  } else if (properties.type === 'Movie') {
    // 观影记录暂时没有详情页，slug 用于让它进入 allPages；未填写时使用 Notion page id。
