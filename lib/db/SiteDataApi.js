...
import {
  ...
  getMockPostBlocks,
  getMockSiteData
} from '@/lib/site/mockData'
// import pLimit from 'p-limit'

export { getAllTags } from './notion/getAllTags'
export function getPost(pageId) {
  return BLOG.USE_MOCK_DATA
    ? getMockPost(pageId)
    : fetchPageFromNotion(pageId)
}

export function getPostBlocks(id, from, options = {}) {
  return BLOG.USE_MOCK_DATA
    ? getMockPostBlocks(id)
...
export async function fetchGlobalAllData({
  ...
  if (BLOG.USE_MOCK_DATA) {
    if (process.env.NODE_ENV === 'development') {
      console.log('[MockData] enabled; Notion API disabled', { from, locale })
    }
    return getMockSiteData({ pageId, from, locale })
  }
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  const data = await getOrSetDataWithCache(cacheKey,
    async () => {
      ...
      // 拉取数据
      const originalPageRecordMap = await getPostBlocks(pageId, from)
      // 转换格式
...
async function getNotice(post) {
  ...
  try {
    const rawBlockMap = await getPostBlocks(post.id, 'data-notice', {
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  if (!post && typeof lastSegment === 'string' && /^[a-f0-9-]{32,36}$/i.test(lastSegment)) {
    ...
    try {
      post = await getPost(lastSegment)
    } catch (e) {
  ...
  const ensureBlockMap = async (post) => {
    ...
    try {
      const rawBlockMap = await getPostBlocks(post.id, source, {
        cacheVersion: post.lastEditedDate
      })
      const adapted = adapterNotionBlockMap(rawBlockMap)
      post.blockMap = {
  ...
  if (post) {
    // 加密文章只在密码验证通过后由客户端请求正文，避免锁定页的初始数据泄露正文。
    if (!post.password) {
